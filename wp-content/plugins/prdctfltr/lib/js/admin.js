...
"use strict";
	...
	$(document).on('change', '.prdctfltr_adv_select', function() {
		...
		if ( curr_el.closest('.adv').hasClass('pf_registered') ) {
			curr_el.closest('.adv').removeClass('pf_registered');
		}
	...
	$(document).on('click', '#prdctfltr_save, #save[type="button"]', function() {
		...
		if ( typeof $('.prdctfltr_customizer_fields').attr('data-lang') !== 'undefined' ) {
			curr_data.lang = $('.prdctfltr_customizer_fields').attr('data-lang');
		}
	...
	$(document).on('change', '.prdctfltr_rng_select', function() {
		...
		if ( curr_el.closest('.rng').hasClass('pf_registered') ) {
			curr_el.closest('.rng').removeClass('pf_registered');
		}
...
console.log(data);
	...
	function get_range_customizations() {
		...
		$('.pf_element.rng textarea[id^="pfr_custom_"]:not(.pfr_registered)').each( function() {
			...
			$(this).closest('td').find('input.pfr_custom_creator').keyup( function () {
				$(this).change();
			});
	...
	function make_title_adv( filter ) {
		filter.find('span:first').text(prdctfltr.localization.adv_filter+' - '+filter.find('select[name^="pfa_taxonomy["] option[value="'+filter.find('select[name^="pfa_taxonomy["]').val()+'"]').text());
	}

	function make_title_rng( filter ) {
		filter.find('span:first').text(prdctfltr.localization.rng_filter+' - '+filter.find('select[name^="pfr_taxonomy["] option[value="'+filter.find('select[name^="pfr_taxonomy["]').val()+'"]').text());
	}

	function reload_filter_titles() {
		$('.pf_element.adv:not(.pf_registered)').each( function(){
			make_title_adv($(this));
			$(this).addClass('pf_registered');
		} );
		$('.pf_element.rng:not(.pf_registered)').each( function(){
			make_title_rng($(this));
			$(this).addClass('pf_registered');
		} );
	}
