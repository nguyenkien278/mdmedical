...
function watu_takings($in_shortcode = false, $atts = null) {
	...
	if($dir != 'ASC' and $dir != 'DESC') $dir = 'DESC';
	...
	if(!empty($_GET['watu_export'])) {
		...
		$rows=array();
		$rows[]=__('User or IP Address', 'watu').';'.__('Date', 'watu').';'.__('Points', 'watu').';'.__('% Correct Answers', 'watu').
			';'.__('Num Correct Answers', 'watu').';'.__('Num Wrong Answers', 'watu').';'.__('Num Unanswered Questions', 'watu').
			';'.__('Result/Grade', 'watu');
		foreach($takings as $taking) {
			$row = ($taking->user_id ? $taking->user_login : $taking->ip).";".date(get_option('date_format'), strtotime($taking->date)).";".
				$taking->points.";".$taking->percent_correct.";".$taking->num_correct.";"
				.$taking->num_wrong.";".$taking->num_empty.";".$taking->result;
			$rows[] = $row;		
		} // end foreach taking
