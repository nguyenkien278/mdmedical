...
if(isset($_REQUEST['do']) and $_REQUEST['do']) { // Quiz Reuslts.
	...
	foreach ($questions as $qct => $ques) {
		...
		$result .= "<div class='show-question-content'>". wpautop($question_number . stripslashes(wp_strip_all_tags($ques->question,'<p>')), true) . "</div>";
		...
		foreach ($all_answers as $ans) {
			...
			$achieved += $points;
			if($ques->answer_type != 'textarea') $result .= wpautop("<li class='$class'><span class='answer'>".(int)$ans->point." - <!--WATUEMAIL".$class."WATUEMAIL-->" . stripslashes($ans->answer) . "</span></li>");
			
			// var_dump($ans->answer.' - '.$ans->point);
		}
	...
	$with_these		= array($achieved,		 $max_points,	  $percent,			$grade,		 $rating,		$num_correct,	$num_wrong,	   stripslashes($quiz_details->name), wpautop(stripslashes($quiz_details->description)), $gtitle, $gdescription, $achieved, $avg_points, $better_than, $num_empty, $num_wrong);
