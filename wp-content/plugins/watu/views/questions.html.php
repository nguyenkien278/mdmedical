<div class="wrap">
	<h2><?php printf(__("Manage Questions in %s", 'watu'), $exam_name); ?></h2>
		...
		<p><a href="admin.php?page=watu_exams"><?php printf(__('Back to %s', 'watu'), WATU_QUIZ_WORD_PLURAL)?></a> &nbsp; <a href="admin.php?page=watu_exam&quiz=<?php echo $_GET['quiz']?>&action=edit"><?php printf(__('Edit this %s', 'watu'), WATU_QUIZ_WORD)?></a> &nbsp;
		...
		if (count($all_question)) {
			...
			foreach($all_question as $question) {
				...
				<td><a href="<?php echo wp_nonce_url('admin.php?page=watu_questions&amp;action=delete&amp;question='.$question->ID.'&amp;quiz='.$_REQUEST['quiz'], 'watu_questions');?>" class='delete' onclick="return confirm('<?php echo addslashes(__("You are about to delete this question. This will delete the answers to this question. Press 'OK' to delete and 'Cancel' to stop.", 'watu'))?>');"><?php _e('Delete', 'watu')?></a></td>
				</tr>
		<?php
				}
			} else {
