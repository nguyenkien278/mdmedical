	...
	class WP_Import_Extend extends WP_Importer {
		...
		function process_menu_item( $item ) {
			...
			if ( $id && ! is_wp_error( $id ) ){
				...
				/** MEGA MENU UPDATE */
				if (isset($mmm_item_descr)) {
					update_post_meta( $id, 'mmm_item_descr', $mmm_item_descr );
				}
				if (isset($mmm_item_style)) {
					update_post_meta( $id, 'mmm_item_style', $mmm_item_style);
				}
				if (isset($mmm_item_visibility)) {
					update_post_meta( $id, 'mmm_item_visibility', $mmm_item_visibility);
				}
				if (isset($mmm_item_icon)) {
					update_post_meta( $id, 'mmm_item_icon', $mmm_item_icon );
				}
				if (isset($mmm_disable_text)) {
					update_post_meta( $id, 'mmm_disable_text', unserialize($mmm_disable_text));
				}
				if (isset($mmm_disable_link)) {
					update_post_meta( $id, 'mmm_disable_link', unserialize($mmm_disable_link) );
				}
				if (isset($mmm_pull_to_other_side)) {
					update_post_meta( $id, 'mmm_pull_to_other_side', unserialize($mmm_pull_to_other_side ));
				}
				if (isset($mmm_submenu_type)) {
					update_post_meta( $id, 'mmm_submenu_type', $mmm_submenu_type );
				}
				if (isset($mmm_submenu_post_type)) {
					update_post_meta( $id, 'mmm_submenu_post_type', $mmm_submenu_post_type);
				}
				if (isset($mmm_submenu_drops_side)) {
					update_post_meta( $id, 'mmm_submenu_drops_side', $mmm_submenu_drops_side );
				}
				if (isset($mmm_submenu_columns)) {
					update_post_meta( $id, 'mmm_submenu_columns', $mmm_submenu_columns);
				}
				if (isset($mmm_submenu_enable_full_width)) {
					update_post_meta( $id, 'mmm_submenu_enable_full_width', unserialize($mmm_submenu_enable_full_width));
				}
				if (isset($mmm_submenu_bg_image)) {
					update_post_meta( $id, 'mmm_submenu_bg_image', unserialize($mmm_submenu_bg_image));
				}
			}
		}
		...
		function process_attachment( $post, $url ) {
			...
			if ( is_wp_error( $upload ) ){
				$this->error_msg =  'Imported '.$url. ' unsuccessfully! Please check the connection';
				return;
			}
