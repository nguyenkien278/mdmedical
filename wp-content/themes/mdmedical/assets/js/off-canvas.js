jQuery(document).ready(function($){
	...
	var container = $('body'),
		...
		bodyClickFn = function(evt) {
			if(menu_states == true &&
				(
					(!$(evt.target).parents('.off-canvas-menu').length && !$(evt.target).hasClass('off-canvas-menu'))
					|| ($(evt.target).attr('id') == 'off-canvas-close')
                    || ($(evt.target).parents('#off-canvas-close').length)
				)
			) {
