...
<?php
	...
	// var_dump($key);
?>
	<?php if (($key == 'mail_sent_ok') || ($key == 'mail_sent_ng')) { ?>
		<p class="description">
			<label for="<?php echo $field_id; ?>"><?php echo esc_html( $arr['description'] ); ?><br />
			<textarea id="<?php echo $field_id; ?>" name="<?php echo $field_name; ?>" class="large-text" size="70" placeholder="<?php echo esc_attr( $post->message( $key, false ) ); ?>" data-config-field="<?php echo sprintf( 'messages.%s', esc_attr( $key ) ); ?>" ><?php echo esc_attr( $post->message( $key, false ) ); ?></textarea>
			</label>
		</p>
	<?php } else { ?>
		<p class="description">
