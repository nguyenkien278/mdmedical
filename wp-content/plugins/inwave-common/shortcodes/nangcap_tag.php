 ...
 * @developer kiennb
 */
if (!class_exists('Inwave_Nangcap_Tag')) {

    class Inwave_Nangcap_Tag extends Inwave_Shortcode{

        protected $name = 'inwave_nangcap_tag';
		
		function getNangcapPost() {
        ...
        function init_params(){	
            return array(
                "name" => __("Tag post Nâng Cấp", 'inwavethemes'),
                "base" => $this->name,
                'category' => 'Custom',
                'icon' => 'iw-default',
                "description" => __("Get tag list in a post Nâng Cấp Làn Da", "inwavethemes"),
                "params" => array(
                    array(
                        "type" => "dropdown",
                        "heading" => "Style",
		...
		ob_start();
			...
			switch ($style){
				case 'style1':
					if ($select_post){
						$terms = get_the_terms($select_post, 'nangcap_tag' );	
						if (!empty($terms)){
							?>
							<div class="single-field-tag nangcap_tag">
