...
class inwave_Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'typography':
					...
					if(isset($typography_stored['height'])) {
						...
						$output .= '<div class="select_wrapper typography-height" original-title="'.esc_attr(esc_html__('Line height', 'mdmedical')).'">';
					...
					if(isset($typography_stored['face'])) {
						...
						$output .= '<div class="select_wrapper typography-face" original-title="'.esc_attr(esc_html__('Font family', 'mdmedical')).'">';
						$output .= '<select class="of-typography of-typography-face select" name="'.esc_attr($value['id']).'[face]" id="'. esc_attr($value['id']).'_face">';
						
						$faces = array('arial'=> esc_html__('Arial', 'mdmedical'),
										'verdana'=> esc_html__('Verdana', 'mdmedical'),
										'trebuchet'=> esc_html__('Trebuchet', 'mdmedical'),
										'georgia' => esc_html__('Georgia', 'mdmedical'),
										'times'=> esc_html__('Times New Roman', 'mdmedical'),
										'tahoma'=> esc_html__('Tahoma, Geneva', 'mdmedical'),
										'palatino'=> esc_html__('Palatino', 'mdmedical'),
										'helvetica'=> esc_html__('Helvetica', 'mdmedical'));
                ...
                // case 'import_button':
                    ...
                    // $output .= '<div class="import-block-title">Select the data you want to import: </div>';
                    // $output .= '<div class="iw-data-list">';
					// $output .= '<div class="data-list">';
                    // $import_types = inwave_get_import_data_types();
                    // foreach($import_types['post-type'] as $data_key => $data_value){
                        // $output .= '<div class="iw-data-option"><input class="post-type" name="post-type-'.esc_attr($data_key).'" type="checkbox" checked="checked" value="1" /> '.$data_value.'</div>';
                    // }
                    // foreach($import_types['data-type'] as $data_key => $data_value){
                        // $output .= '<div class="iw-data-option"><input class="data-type" name="data-type-'.esc_attr($data_key).'" type="checkbox" checked="checked" value="1" /> '.$data_value.'</div>';
                    // }
                    // $output .= '</div>';
					// $output .= '<div class="system-required">';
					// $output .= '<h3>' . esc_html__('System Requirements', 'mdmedical') . '</h3>';
				...
				case 'slider':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">'.esc_html__('Add New Slide', 'mdmedical').'</a></div>';
				...
				case 'addoption':
					...
					$label = isset($value['option_label']) ? $value['option_label'] : esc_html__('Option', 'mdmedical');
					$add_btn_text = isset($value['add_btn_text']) ? $value['add_btn_text'] : esc_html__('Add New Option', 'mdmedical');
					$i = 0;
					if ($count > 1) {
						unset($addoptions[0]);
				...
				case 'social_link':
					...
					$addoptions = $inwave_theme_option[$value['id']];
					$count = count($addoptions);
					$label = isset($value['option_label']) ? $value['option_label'] : esc_html__('Social', 'mdmedical');
					$add_btn_text = isset($value['add_btn_text']) ? $value['add_btn_text'] : esc_html__('Add New Social', 'mdmedical');
					$i = 0;
					if ($count > 1) {
						unset($addoptions[0]);
						foreach ($addoptions as $addoption) {
							...
							$i++;
							$order = $i;
							$output .= inwave_Options_Machine::optionsframework_social_link_function($value['id'], $value['std'], $oldorder, $order);
						}
					}
					$output .= '</ul>';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ','mdmedical').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button">'.esc_html__('Backup Options','mdmedical').'</a>';
					$output .= '<a href="#" id="of_restore_button" class="button">'.esc_html__('Restore Options','mdmedical').'</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button">'.esc_html__('Import Options','mdmedical').'</a>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.esc_attr($id).'">'.esc_html__('Upload', 'mdmedical').'</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. esc_attr($hide).'" id="reset_'. esc_attr($id) .'" title="' . esc_attr($id) . '">'.esc_html__('Remove', 'mdmedical'). '</span>';
		}
		else 
		{
			$output .= '<p class="upload-notice"><i>'.esc_html__('Upgrade your version of WordPress for full media support', 'mdmedical').'</i></p>';
		}
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>'.esc_html__('Slide', 'mdmedical').' '.$order.'</strong>';
		}
		...
		$slider .= '<a class="slide_edit_button" href="#">'.esc_html__('Edit','mdmedical').'</a></div>';
		
		$slider .= '<div class="slide_body">';
		
		$slider .= '<label>'.esc_html__('Title','mdmedical').'</label>';
		...
		$slider .= '<label>'.esc_html__('Image URL','mdmedical').'</label>';
		...
		$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.esc_attr($id.'_'.$order) .'">'.esc_html__('Upload','mdmedical').'</span>';
		
		if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
		$slider .= '<span class="button remove-image '. esc_attr($hide).'" id="reset_'. esc_attr($id .'_'.$order) .'" title="' . esc_attr($id . '_'.$order) .'">'.esc_html__('Remove','mdmedical').'</span>';
		$slider .='</div>' . "\n";
		...
		$slider .= '</div>';	
		$slider .= '<label>'.esc_html__('Link URL (optional)', 'mdmedical'). '</label>';
		...
		$slider .= '<label>'.esc_html__('Description (optional)', 'mdmedical').'</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
	
		$slider .= '<a class="slide_delete_button" href="#">'.esc_html__('Delete','mdmedical').'</a>';
	...
	public static function optionsframework_addoption_function($id,$std,$label,$oldorder,$order){
		...
		$addoption .= '<a class="slide_edit_button" href="#">'.esc_html__('Edit','mdmedical').'</a></div>';
		...
		$addoption .= '<a class="slide_delete_button" href="#">'.esc_html__('Delete','mdmedical').'</a>';
	 ...
	 * @return string
	 */
	public static function optionsframework_social_link_function($id,$std,$oldorder,$order){
	...
	static function getSystemRequireStatus() {
        ...
        <ul class="check-system-status">
            <li>
                <?php
                echo wp_kses(__('<span class="label1">PHP Version: </span>', 'mdmedical'), inwave_allow_tags('span'));
                if ($php_version < '5.3.0') {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'mdmedical'), inwave_allow_tags('span')), $php_version) . '(' . esc_html__('Min: 5.3', 'mdmedical') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'mdmedical'), inwave_allow_tags('span')), $php_version);
                }
                ?>
            </li>
            <li>
                <?php
                echo wp_kses(__('<span class="label1">Maximum execution time: </span>', 'mdmedical'), inwave_allow_tags('span'));
                if ($max_execution_time < '90') {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'mdmedical'), inwave_allow_tags('span')), $max_execution_time) . '(' . esc_html__('Min: 90', 'mdmedical') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'mdmedical'), inwave_allow_tags('span')), $max_execution_time);
                }
                ?>
            </li>
            <li>
                <?php
                echo wp_kses(__('<span class="label1">Memory Limit: </span>', 'mdmedical'), inwave_allow_tags('span'));
                if ($memory_limit < '128M') {
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'mdmedical'), inwave_allow_tags('span')), $memory_limit) . '(' . esc_html__('Min: 128M', 'mdmedical') . ')';
                } else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>' . sprintf(wp_kses(__('<span class="label2">Currently: %s</span>', 'mdmedical'), inwave_allow_tags('span')), $memory_limit);
                }
                ?>
            </li>
            <li>
                <?php
                echo wp_kses(__('<span class="label1">Uploads folder writable: </span>', 'mdmedical'), inwave_allow_tags('span'));
			...
			<li>
				<?php
				echo wp_kses(__('<span class="label1">Connect InwaveThemes Server: </span>', 'mdmedical'), inwave_allow_tags('span'));
				if (!$file_get_content) {
					echo '<span class="invalid"><i class="fa fa-times"></i></span>';
				} else {
					echo '<span class="valid"><i class="fa fa-check"></i></span>';
            ...
            <li>
                <?php
                echo wp_kses(__('<span class="label1">Start Session: </span>', 'mdmedical'), inwave_allow_tags('span'));

                if (!session_id()) {
                    session_start();
                }
                if(!session_id()){
                    echo '<span class="invalid"><i class="fa fa-times"></i></span>' . wp_kses(__('<span class="label2">Failed to start session. Please check session save_path in your php setting</span>', 'mdmedical'), inwave_allow_tags('span'));
                }else {
                    echo '<span class="valid"><i class="fa fa-check"></i></span>';
                }
                ?>
